...
pub async fn start_server(
    ...
    let inner = Router::new()
        ...
        .route("/api/submodules/{name}/packages/{pkg}", get(api_submodule_package))
        ...
        .layer(middleware::from_fn_with_state(state.clone(), spinner_middleware))
...
async fn api_media_manifest(
    ...
    let manifest_path = state.output_dir.join("media").join(&date).join("manifest.json");
...
async fn api_media_file(
    ...
    match safe_resolve(&media_dir, &candidate).await {
        Some(resolved) if resolved.is_file() => {
            match fs_ops::read_file_bytes(&resolved).await {
                Ok(bytes) => {
...
fn render_markdown_with_toc(input: &str) -> (String, Vec<TocEntry>) {
    ...
    for event in parser {
        match event {
            ...
            Event::End(TagEnd::Heading(level)) if level == HeadingLevel::H2 || level == HeadingLevel::H3 => {
                ...
                html.push_str(&format!("</h{}>", match level {
                    HeadingLevel::H2 => 2,
                    HeadingLevel::H3 => 3,
                    _ => 2,
                }));
    ...
    for line in content.lines() {
        ...
        if lower.contains("commit") {
            ...
            for word in lower.split_whitespace() {
                if let Ok(n) = word.trim_start_matches('*').trim_start_matches('-').parse::<u32>() {
                    if n > 0 && n < 10000 {
                        return n;
                    }
                }
