...
function useCurrentPath(): string {
  ...
  useEffect(() => {
    const handler = (): void => setPath(window.location.pathname);
      ...
      <div class="flex items-center">
        ...
        <div class={hasPackages ? '' : 'pl-5'} style={{ flex: '1', minWidth: 0 }}>
          <NavLink
            ...
            active={currentPath === `/submodules/${sub.name}`}
          >
            {sub.name}
          </NavLink>
        </div>
  ...
  return (
    <nav class="w-64 min-w-[16rem] bg-latte-mantle dark:bg-mocha-mantle border-r border-latte-surface0 dark:border-mocha-surface0 p-4 sticky top-0 h-screen overflow-y-auto shrink-0">
      ...
      {nav?.docs && nav.docs.length > 0 ? (
        ...
        <>
          <NavLink href="/docs/overview" active={currentPath === '/docs/overview'}>Overview</NavLink>
          <NavLink href="/docs/architecture" active={currentPath === '/docs/architecture'}>Architecture</NavLink>
          <NavLink href="/docs/apis" active={currentPath === '/docs/apis'}>APIs</NavLink>
          <NavLink href="/docs/databases" active={currentPath === '/docs/databases'}>Databases</NavLink>
          <NavLink href="/docs/dependencies" active={currentPath === '/docs/dependencies'}>Dependencies</NavLink>
        </>
      )}
