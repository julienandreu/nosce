...
import { TAG_COLORS, REPO_COLORS } from '../constants';
...
export function ReportsList(_props: ReportsListProps): preact.JSX.Element {
  ...
  const allRepos = useMemo(() => {
    const set = new Set<string>();
    reports.forEach((r) => r.repos.forEach((rp) => set.add(rp)));
    return Array.from(set).sort();
  }, [reports]);
  ...
  const filtered = useMemo(() => {
    return reports.filter((r) => {
      ...
      if (activeRepoFilter && !r.repos.includes(activeRepoFilter)) return false;
      return true;
    });
      ...
      {(allTags.length > 0 || allRepos.length > 0) && (
        <div class="mb-6 flex flex-wrap items-center gap-2">
          ...
          {allRepos.map((repo) => (
            <button
              key={repo}
              onClick={() => setActiveRepoFilter(activeRepoFilter === repo ? null : repo)}
        ...
        <div class="text-center py-16 text-latte-subtext0 dark:text-mocha-subtext0">
          {reports.length === 0 ? (
            <>
              ...
              <p class="text-sm">
                Run{' '}
                <code class="bg-latte-surface0 dark:bg-mocha-surface0 px-2 py-0.5 rounded">
                  /sync
                </code>{' '}
                to generate reports.
              </p>
            ...
            >
              <div class="p-4">
                ...
                <div class="flex items-center gap-3 mb-2">
                  <span class="text-lg font-semibold text-latte-text dark:text-mocha-text">
                    {r.label}
                  </span>
                  <span class="text-sm text-latte-overlay0 dark:text-mocha-overlay0">
                    {r.date_range}
                ...
                <div class="flex flex-wrap gap-1.5">
                  ...
                  {r.tags.map((tag) => (
                    <Tag
                      key={tag}
                      label={tag}
                      color={TAG_COLORS[tag] ?? 'bg-latte-surface0 dark:bg-mocha-surface0'}
                    />
                  ))}
                </div>
              </div>
