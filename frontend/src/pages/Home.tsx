    ...
    <Card>
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-lg bg-latte-blue/10 dark:bg-mocha-blue/10 flex items-center justify-center">
          <svg class="w-5 h-5 text-latte-blue dark:text-mocha-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-3xl mx-auto">
      ...
      {currentProfile && (
        <div class="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 class="text-xl font-bold text-latte-text dark:text-mocha-text">
              {currentProfile.label} Dashboard
            </h1>
            <p class="text-sm text-latte-subtext0 dark:text-mocha-subtext0">
      ...
      {latestReport && (
        ...
        >
          <div class="flex items-center gap-3 mb-2">
            <h2 class="text-lg font-semibold text-latte-text dark:text-mocha-text">
              Latest: {latestReport.label}
            </h2>
            <span class="text-sm text-latte-overlay0 dark:text-mocha-overlay0">
              {latestReport.date_range}
            </span>
          </div>
          {latestReport.tldr && (
            <p class="text-sm text-latte-subtext0 dark:text-mocha-subtext0 mb-3 line-clamp-2">
              {latestReport.tldr}
            </p>
          )}
          ...
          <div>
            ...
            <div class="space-y-2">
              {data.docs.map((cat) => (
                ...
                >
                  <svg class="w-5 h-5 text-latte-blue dark:text-mocha-blue shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d={DOC_ICONS[cat] ?? DOC_ICONS['overview'] ?? ''} />
                  </svg>
            ...
            <div class="space-y-2">
              {data.submodules.map((sub) => (
                <a
                  ...
                  class="flex items-center gap-3 p-3 rounded-lg border border-latte-surface0 dark:border-mocha-surface0 bg-latte-mantle dark:bg-mocha-mantle hover:border-latte-blue/50 dark:hover:border-mocha-blue/50 transition-all"
                >
                  <svg class="w-5 h-5 text-latte-teal dark:text-mocha-teal shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
