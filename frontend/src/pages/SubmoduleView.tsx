...
interface SubmoduleViewProps extends RoutableProps {
  name?: string;
}

export function SubmoduleView({ name }: SubmoduleViewProps): preact.JSX.Element | null {
  ...
  const url = apiUrl(`/api/submodules/${name}?profile=${encodeURIComponent(profileId)}`);
