...
function groupByRepoPr(items: MediaItem[]): GroupedMedia[] {
  ...
  for (const item of items) {
    ...
    if (!prMap.has(item.pr_number)) {
      prMap.set(item.pr_number, { pr_title: item.pr_title, author: item.author, items: [item] });
    }
...
export function MediaGallery({ date }: MediaGalleryProps): preact.JSX.Element | null {
  ...
  if (loading || !manifest || manifest.items.length === 0) {
    return null;
  }
  ...
  const parts: string[] = [];
  if (imageCount > 0) {
    parts.push(`${String(imageCount)} image${imageCount !== 1 ? 's' : ''}`);
  }

  if (videoCount > 0) {
    parts.push(`${String(videoCount)} video${videoCount !== 1 ? 's' : ''}`);
  }
  ...
  return (
    <>
      <div class="media-gallery">
        <div
          class="media-gallery-header"
          onClick={() => {
            setExpanded(!expanded);
          }}
        >
          <div class="flex items-center gap-2">
            <span class="text-xs transition-transform" style={{ transform: expanded ? 'rotate(90deg)' : 'rotate(0)' }}>
              &#9654;
            </span>
            <span class="text-sm font-medium text-latte-text dark:text-mocha-text">Screenshots & Videos</span>
          ...
          <div>
            {groups.map((group) => (
              <div key={group.repo}>
                ...
                {group.prs.map((pr) => (
                  <div key={`${group.repo}-${String(pr.pr_number)}`} class="media-pr-group">
                        ...
                        item.type === 'image' ? (
                          ...
                          >
                            <img
                              ...
                              loading="lazy"
                            />
                            <div class="media-thumbnail-caption">{item.alt || item.filename}</div>
                          </div>
                        ) : (
